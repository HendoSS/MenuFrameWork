...
/*Given year,month,day returns day of month Sunday: 0 Monday: 1, 9 is error
 * code*/
int Calendar::DayOfWeek(int year, int month, int day) {
  // from wikipedia
  if (!day <= DaysInMonth(year, month))
    std::cout
        << "Day is over max number of days in that month, in DayOfWeek calc \n";
...
XMFLOAT2 Calendar::DrawHeader(XMFLOAT2 pos) {
  XMFLOAT2 size =
      appinstance->DrawString(pos, .9f, false, "Su Mo Tu We Th Fr Sa");
  HeaderPos = pos;
  HeaderSize = size;
  return size;
}
